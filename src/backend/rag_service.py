...
class RagService:
    ...
    def _page_label(self, metadata: dict[str, Any]) -> str:
        ...
        if page is None or page == "":
            return "N/A"
        try:
            return str(int(page) + 1)   # 0-indexed → 1-indexed
    ...
    def _get_rerank_meta(self) -> dict:
        ...
        Returns a dict with rerank_* keys (all safe defaults if not present).
        """
    ...
    def answer(
        ...
        if not question:
            return self.build_response(
                question=question, answer="", docs=[], latency=time.time() - start
            )
        ...
        if not docs:
            ...
            result = self.build_response(
                question=question, answer=msg, docs=[], latency=time.time() - start
            )
